<li class="nav-item  @if (count($lista) > 0) dropdown @endif">
    <a class="nav-link @if (count($lista) > 0) dropdown-toggle @endif {{ request()->segment(2) == $segmentoLink ? 'active show' : '' }}"
        @if (count($lista) > 0) data-bs-toggle="dropdown" @endif href="{{ route($ruta) }}" role="button"
        aria-expanded="false">
    ...
    @if (count($lista) > 0)
        <ul class="dropdown-menu {{ request()->segment(2) == $segmentoLink ? 'show' : '' }}">
            @foreach ($lista as $nombre => $link)
                @isset($link[2])
                    <li><a class="dropdown-item nav-link" href="{{ route($link[0], $link[2]) }}">
                        @endisset
                        @empty($link[2])
