<div>
    <div>
        <div class="row">
            ...
            @else
                <div class="shadow-sm mb-4 prestamo-lista" type="button" id=""
                    wire:click="mostrarPrestamoSingle('{{ $caja }}')">
                    <div class="card theme-bg text-white text-center">
                        <div class="card-body">
...
@push('footer')
        ...
        Livewire.on('mostrarSemanaMany', data => {
            ...
            if (data.length == 0) {
                $('.lista-prestamo-many').append(
                    '<p class="text-muted">No hay prestamos registrados esta semana</p>');
            } else {
                data.forEach(function(prestamo, index) {
                    ...
                    $('.lista-prestamo-many').append(
                        ...
                        prestamo.monto_inicial + ' Bs.</p><p class="text-muted size-12">' + moment(
                            fecha)
    ...
    <script>
        ...
        Livewire.on('mostrarSemanaSingle', datas => {
            ...
            var diaAnterior = null;
            if (datos.length == 0) {
                $('.lista-prestamo-single').append('<p class="text-muted">No existen prestamos registrados en esta semana</p>');
            } else {
